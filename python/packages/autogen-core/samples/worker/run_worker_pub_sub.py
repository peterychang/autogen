...
from messages_pb2 import Greeting, AskToGreet, Feedback, ReturnedFeedback, ReturnedGreeting
...
class ReceiveAgent(RoutedAgent):
    ...
    async def on_greet(self, message: Greeting, ctx: MessageContext) -> None:
        await self.publish_message(ReturnedGreeting(content=f"Returned greeting: {message.content}"), topic_id=DefaultTopicId())

    @message_handler
    async def on_feedback(self, message: Feedback, ctx: MessageContext) -> None:
        await self.publish_message(ReturnedFeedback(content=f"Returned feedback: {message.content}"), topic_id=DefaultTopicId())
